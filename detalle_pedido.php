...
$sql = "SELECT * FROM detalles_pedido WHERE pedido_id = $pedido_id";
$result = $conn->query($sql);

// Consulta para obtener la información del pedido
$sql_pedido = "SELECT * FROM pedidos WHERE pedido_id = $pedido_id";
...
<head>
    <meta charset="utf-8">
    <title>Detalles de Pedido | POOMSTORE</title>
  ...
  <div class="container">
    <div class="columns">
      <div class="column is-11-desktop is-offset-1-desktop">
        <h1 class="has-text-white font-tertiary">Detalles del Pedido</h1>
      </div>
    </div>
  </div>
</section>
...
<section class="section">
    <div class="container">
        <?php
        if ($result->num_rows > 0 && $pedido) {
            $total = $pedido['total'];
            echo '<h2 class="title">Pedido ID: ' . $pedido_id . '</h2>';
            echo '<p>Fecha: ' . date('d-m-Y H:i:s', strtotime($pedido['fecha_pedido'])) . '</p>';
            ...
            while ($detalle = $result->fetch_assoc()) {
                $nombre_producto = htmlspecialchars($detalle['nombre_producto']);
            ...
            echo '<div class="notification is-warning">No se encontraron detalles para este pedido.</div>';
        }
