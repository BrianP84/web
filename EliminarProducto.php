...
<head>
  <meta charset="utf-8">
  <title>Eliminar Producto | POOMSTORE</title>
...
$buttonDisabled = 'disabled-button'; // Inicialmente deshabilitar el botón
...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $buttonDisabled = ''; // Habilitar botón si se encuentra el producto
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($producto_id) {
        ...
        if ($stmt->execute()) {
            $message = "<div class='notification is-primary'>Producto eliminado exitosamente.</div>";
            $nombre = "";
            $descripcion = "";
            $precio = "";
            $categoria_id = "";
            ...
            $buttonDisabled = 'disabled-button'; // Deshabilitar botón después de eliminar
        } else {
            $message = "<div class='notification is-danger'>Error al eliminar producto: " . $stmt->error . "</div>";
        }
  ...
  <div class="container">
    <div class="columns">
      <div class="column is-11-desktop is-offset-1-desktop">
        <h1 class="has-text-white font-tertiary">Eliminar Producto</h1>
      </div>
    </div>
  </div>
</section>
      ...
      <div class="column is-half">
        ...
        <form action="" method="post" class="box">
          ...
          <div class="field">
            ...
            <div class="control">
              <input class="input" type="text" name="nombre" value="<?php echo htmlspecialchars($nombre); ?>" readonly class="<?php echo $fieldsDisabled; ?>">
            </div>
          </div>
          <div class="field">
            <label class="label">Descripción</label>
            <div class="control">
              <textarea class="textarea" name="descripcion" readonly class="<?php echo $fieldsDisabled; ?>"><?php echo htmlspecialchars($descripcion); ?></textarea>
            </div>
          </div>
          <div class="field">
            <label class="label">Precio</label>
            <div class="control">
              <input class="input" type="number" name="precio" step="0.01" value="<?php echo htmlspecialchars($precio); ?>" readonly class="<?php echo $fieldsDisabled; ?>">
            </div>
          </div>
          <div class="field">
            <label class="label">Categoría ID</label>
            <div class="control">
              <input class="input" type="number" name="categoria_id" value="<?php echo htmlspecialchars($categoria_id); ?>" readonly class="<?php echo $fieldsDisabled; ?>">
            </div>
          </div>
          <div class="field">
            <div class="control">
              <button class="button is-danger <?php echo $buttonDisabled; ?>" type="submit" <?php echo ($fieldsDisabled === 'disabled-field') ? 'disabled' : ''; ?>>Eliminar Producto</button>
            </div>
          </div>
        </form>
        ...
        <?php if ($message): ?>
          <div class="notification is-primary"><?php echo $message; ?></div>
        <?php endif; ?>
      </div>
    </div>
  </div>
...
<!-- bulma -->
<script src="plugins/bulma/bulma.min.js"></script>
<!-- slick slider -->
<script src="plugins/slick/slick.min.js"></script>
<!-- themefy-icon -->
<script src="plugins/themify-icons/themify-icons.js"></script>
<!-- main -->
